...
function renderChartInfoAcquisto(logs) {
    ...
    const acquistoColors = ['#4a90d9', '#ff3d3d', '#00bcd4'];
    logs.forEach(log => {
    ...
    contactChartAcquisto = new Chart(ctx.getContext('2d'), {
        type: 'doughnut',
        data: { labels: tipologie, datasets: [{ data: tipologie.map(t => counts[t]), backgroundColor: acquistoColors, borderWidth: 2, borderColor: isDark ? '#0d0f1a' : '#ffffff' }] },
        options: {
            responsive: true, maintainAspectRatio: true,
            plugins: {
                legend: {
        ...
        <div class="contact-day-section">
            <div class="contact-table-wrapper">
                <table class="contact-table">
                    ...
                    <tbody>
                        ${items.length > 0 ? items.map(log => renderContactRow(log)).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--text-secondary);padding:20px">Nessun contatto</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>`;
    ...
    return `
        <div class="contact-day-cards-grid">
            ${weekDates.map((date, idx) => {
                ...
                return `
                <div class="contact-day-card ${hasData ? 'contact-day-card-active' : 'contact-day-card-empty'}" style="${bgStyle}" ${hasData ? `onclick="showDayView('${date}')"` : ''}>
...
function printDay(date) {
    ...
    win.document.write(`<html><head><title>Registro ${date}</title><style>body{font-family:Arial,sans-serif;font-size:12px;padding:20px}h2{font-size:16px;margin-bottom:10px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:6px 10px;text-align:left}th{background:#f0f0f0;font-weight:700;font-size:10px;text-transform:uppercase}@page{margin:15mm}</style></head><body>
        ...
        <table><thead><tr><th>Orario</th><th>Categoria</th><th>Dettaglio</th><th>Operatore</th></tr></thead><tbody>
        ${dayLogs.map(log => `<tr><td>${log.contactDate.split('T')[1].substring(0,5)}</td><td>${log.category}</td><td>${log.otherNote||'—'}</td><td>${log.user.fullName}</td></tr>`).join('')}
...
function selectAcquisto(tipo) {
    ...
    const keyMap = { 'Info Consegna': 'InfoConsegna', 'Ritardo Consegna': 'RitardoConsegna', 'Info Documentazione': 'InfoDocumentazione' };
